class LensAddonsService {
  constructor(lensAddonsRepository, tenantRepository) {
    this.repository = lensAddonsRepository;
  ...
  async getAllPaginated(user, filters, db) {
    const tenantId =
      user.role === "super_admin" ? filters.tenant_id || null : user.tenant_id;
    const { addons, totalCount } = await this.repository.getPaginatedByTenantId(
      db,
      tenantId,
      filters,
    );
    ...
    return { data: addons, count: totalCount, page_count };
  }
  ...
  async create(data, user, db) {
    ...
    try {
      ...
      const newAddon = await this.repository.create(client, {
        ...data,
        tenant_id: tenantId,
      });
      await client.query("COMMIT");
      return { status: "success", data: newAddon };
    } catch (error) {
  ...
  async getById(id, user, db) {
    ...
    const addon = await this.repository.getById(db, id, tenantId);
    if (!addon)
      throw Object.assign(new Error("Lens Addon not found"), {
        statusCode: 404,
      });
    return addon;
  }
...
module.exports = LensAddonsService;

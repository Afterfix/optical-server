class FrameVariantRepository {
  ...
  async create(db, data) {
    const { tenant_id, frame_id, color, size, sku, stock_qty } = data;
    const { rows } = await db.query(
      `INSERT INTO frame_variants (tenant_id, frame_id, color, size, sku, stock_qty)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [tenant_id, frame_id, color, size, sku, stock_qty],
    );
    return rows[0];
  }

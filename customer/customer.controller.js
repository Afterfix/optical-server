class CustomerController {
  ...
  async update(req, res, next) {
    try {
      const updated = await this.service.update(req.params.id, req.body, req.user, req.db);
      updated ? res.json({ status: 'success', data: updated }) : res.status(404).json({ message: "Not found" });
    } catch (e) { next(e); }
  }
